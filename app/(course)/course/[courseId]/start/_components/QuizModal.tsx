...
// Server-side updates are performed via API endpoints
  ...
  const handleNext = async () => {
    if (currentQuestionIndex < questions.length - 1) {
      ...
      try {
        ...
        const resp = await fetch(`/api/courses/${encodeURIComponent(courseId)}/progress`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ increment: progressIncrement }),
        })
        const json = await resp.json()
        const newProgress = json?.data?.progress ?? null
        if (newProgress !== null && newProgress >= 100 - (100 / totalChapters)) {
