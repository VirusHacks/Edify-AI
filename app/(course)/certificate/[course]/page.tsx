...
export default  function CertificatePage({ params }: { params: { course: string } }) {
 ...
 const [course, setCourse] = useState<CourseType | null>(null);
  ...
  const resp = await fetch(`/api/courses/${encodeURIComponent(params.course)}`)
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-white">
      <PageContainer className="py-8 flex flex-col items-center justify-center">
        <div 
          ref={certificateRef} 
          className="bg-white w-[856px] h-[578px] relative shadow-2xl mx-auto rounded-2xl overflow-hidden"
          style={{
            backgroundImage: 'radial-gradient(circle at center, rgba(129, 140, 248, 0.05) 0%, transparent 70%)'
          }}
        >
        {/* Double Border */}
          ...
          <div className="mt-12 w-full">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {recommendations.map((course, index) => (
                <Card key={index} className="bg-white rounded-2xl border border-slate-200 shadow-sm hover:shadow-md transition-shadow duration-300">
                  <CardContent className="p-6">
                    ...
                    <div className="flex justify-between items-center text-xs text-slate-500">
                      <span className="px-2 py-1 bg-slate-100 rounded-lg">{course.category}</span>
        ...
        <Button 
          ...
          className="mt-8 bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-700 hover:to-purple-700 text-white px-8 py-4 rounded-2xl text-lg font-semibold shadow-lg hover:shadow-xl transition-all duration-200 hover:scale-105 flex items-center gap-3"
        >
        {isDownloading ? (
          <>
