import {
  ...
  export const pathways = pgTable('pathways', {
    ...
    userId: text('user_id'), // null for public paths, user ID for personalized paths
    isPersonalized: boolean('is_personalized').default(false),
    targetCareer: text('target_career'),
    marketInsights: jsonb('market_insights').$type<{
      demandTrends: string[];
      salaryRange: string;
      topCompanies: string[];
    ...
    personalizedFor: jsonb('personalized_for').$type<{
